...
<body>
    <header><h1>Najdroższe czujniki</h1></header>
    <div class="head">
    <?php
        $counter = $conn->query(" SELECT COUNT(ID) as 'ile' FROM arkusz having max(cenaczujnika)");
    ...
    <main>
        <ul>
        <?php
            $records = $conn->query(" SELECT arkusz.ID, arkusz.Nazwa, arkusz.CenaCzujnika FROM arkusz having max(cenaczujnika)");
        ...
        <?php
            $sum = $conn->query("SELECT Sum(CenaCzujnika) as 'suma' from arkusz having max(cenaczujnika)");
